...
public class AsynchronousFileDemo {
    ...
    public void testCompletionHandler() throws IOException, InterruptedException {
        ...
        AsynchronousFileChannel ch = AsynchronousFileChannel.open(
                Paths.get(ROOT_PATH, "/test/test/test.txt"), StandardOpenOption.READ);
        //读取文件临时ByteBuffer对象
        ...
        while (true) {
            ...
            //如果读取完毕,则直接退出
            if (byteReadResultHolder.size() <= 0 || byteReadResultHolder.get(0) == -1) {
                break;
            }
    ...
    public void test2() throws IOException, InterruptedException {
        //若文件p.txt不存在,则创建该文件
        Path path = Paths.get(ROOT_PATH, "/test/test/test.txt");
